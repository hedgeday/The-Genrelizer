...
function getTag(result) {
    ...
    else
    {
        ...
        currentSearch.url = result.artist.url;
...
function getTracks(result) {
    ...
    else
    {
    	...
    	for (var i = 0; i < currentSearch['tracks'].length;i++){
    		$('#topTracks').html($('#topTracks').html() + '<a target="_blank" href="' + currentSearch["url"] + '">' + currentSearch['tracks'][i]['artist']['name'] + ' : ' +
    		currentSearch['tracks'][i]['name'] + '</a>');
        }
...
function getWeeklyChart(result){
	...
	for (var i = 0; i < currentSearch.weeklyChart.length;i++){
        if (i === 3)
            $('#topCharts').html($('#topCharts').html() + '<div class="artistPicDiv clear"><img src="' + currentSearch.weeklyChart[i]['image'][2]['#text'] + '"></img>' + '<p class="center">' + currentSearch.weeklyChart[i]['name'] + '</p></div>');
    	else
    ...
    $('#topCharts').html($('#topCharts').html() + '<h2 class="clear">Artist popularity:</h2>');
    searchedFlag=false;
    resizeAll();
    counter++; 
